 ...
 *	Description: In one state machine (SM), output to a shared variable (threeLEDs) 
 *	the following behavior: set only bit 0 to 1, then only bit 1, then only bit 2 
 *	in sequence for 1 second each.
 *	In a second state machine (BlinkingLEDSM), output to a shared variable 
 *	(blinkingLED) the following behavior: set bit 3 to 1 for 1 second, then 0 for
 *	1 second.
 *	In a third state machine (CombineLEDsSM), combine both shared variables and 
 *	output to the PORTB.
